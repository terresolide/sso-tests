 ...
 * under the License.
 * 
 * Class modified by Aeris 
 * Class modified by ForM@Ter to use composer and phpseclib repository
 *
 */
...
class OpenIDConnectClient
{
    ...
    /**
     * EDITED
     * @var home URL for log out redirect
     */
    private $homeURL;

    /**
     * EDITED
     * @var unauthorized redirect URL
     */
    private $unauthorizedURL;

    private $enc_type = 1; //PHP_QUERY_RFC1738
     ...
     * @param null $issuer
     */
    public function __construct($provider_url = null, $client_id = null, $client_secret = null, $homeURL = null, $unauthorizedURL = null, $issuer = null) {
    ...
    /**
     * Edited by dbernard - Thales Services
    */
    public function issuerValidator($iss){
	        return ($iss === $this->getIssuer() || $iss === $this->getWellKnownIssuer() || $iss === $this->getWellKnownIssuer(true));
    }
    ...
    public function signOut($accessToken, $redirect) {
        ...
        $signout_params = null;
        if($redirect === null){
            $signout_params = array('id_token_hint' => $accessToken, 'redirect_uri' => $this->homeURL);
        }
        else {
            $signout_params = array(
                'id_token_hint' => $accessToken,
                'post_logout_redirect_uri' => $redirect,
		        'redirect_uri' => $this->homeURL);
        }
	...
	*/
        ...
        if (isset($_SERVER['HTTP_UPGRADE_INSECURE_REQUESTS']) && ($_SERVER['HTTP_UPGRADE_INSECURE_REQUESTS'] === '1') && false) {
            $protocol = 'https';
        } else {
        ...
        $port = @intval($_SERVER['HTTP_X_FORWARDED_PORT'])
            ? @intval($_SERVER['HTTP_X_FORWARDED_PORT']) : @intval($_SERVER['SERVER_PORT'])
                ? @intval($_SERVER['SERVER_PORT']) : (($protocol === 'https') ? 443 : 80);

	$explodeHttpsHost = @explode(':', $_SERVER['HTTP_HOST']);
        $host = count($explodeHttpsHost) > 0
            ? array_shift($explodeHttpsHost) : @$_SERVER['SERVER_NAME']
    ...
    // EDITED: new function to build path for GET queries
    public function http_build_query_get($auth_endpoint, $auth_params) {
	// EDITED: Previous line not working...
	$forCount = 0;
    ...
    /**
     * Requests for Keycloak authorization
     * writted by dbernard - Thales Services
     *
     * @throws OpenIDConnectClientException
     */
    public function checkKeycloakAuthorization() {
	...
	$post_params = http_build_query($post_data, null, '&');
        ...
        return $this->fetchURL($token_endpoint, $post_params, $headers, true);
    }
    ...
    private function requestTokens($code) {
        $token_endpoint = $this->getProviderConfigValue('token_endpoint');
        $token_endpoint_auth_methods_supported = $this->getProviderConfigValue('token_endpoint_auth_methods_supported', array('client_secret_basic'));
    ...
    private function verifyJWTclaims($claims, $accessToken = null) {
        ...
        return (($this->issuerValidator($claims->iss))
     ...
     * @return mixed
     */
    protected function fetchURL($url, $post_body = null, $headers = array(), $redirectOnUnauthorized = false) {
	...
	// EDITED
	if ($this->responseCode === 403 && $redirectOnUnauthorized && $this->unauthorizedURL !== null) {
    ...
    {
        $len1= safeLength($str1);
        $len2= safeLength($str2);
